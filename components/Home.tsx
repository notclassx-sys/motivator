...
export const Home: React.FC<HomeProps> = ({ planners, onToggleTask }) => {
  const [quote, setQuote] = useState("One step at a time.");
    ...
    <div className="space-y-6 animate-in fade-in duration-500">
      <div className="flex items-center justify-between px-5 py-3 rounded-2xl bg-[#1C1C1E] border border-white/5">
        <div className="flex items-center space-x-3">
          ...
          <span className="text-[10px] font-bold text-white uppercase tracking-widest">Sync Active</span>
        </div>
        <div className="flex items-center space-x-2 px-3 py-1 rounded-full bg-[#3B82F6]/10 border border-[#3B82F6]/20">
          ...
          <span className="text-[8px] font-bold text-[#3B82F6] uppercase tracking-widest">Safe</span>
        </div>
      </div>
      ...
      <header className="flex justify-between items-end px-1">
        <div>
          ...
          <p className="text-[#A1A1AA] text-[10px] font-bold uppercase tracking-widest mt-1">Today's Overview</p>
        </div>
      ...
      <div className="relative rounded-3xl p-8 bg-[#1C1C1E] border border-white/5 shadow-xl">
        <div className="flex items-center space-x-2 mb-6">
          <Sparkles size={14} className="text-[#C5A059]" />
          <span className="text-[10px] font-bold text-[#C5A059] uppercase tracking-widest">Daily Motivation</span>
        </div>
        <p className="text-xl font-bold leading-tight mb-8 text-white/95">"{quote}"</p>
        <div className="flex space-x-3">
          <button 
            onClick={fetchQuote} 
            disabled={isRefreshingQuote}
            className="flex-1 bg-[#3B82F6] text-white py-4 rounded-2xl font-bold text-[11px] uppercase tracking-widest active:scale-95 transition-all shadow-lg"
          >
            {isRefreshingQuote ? 'Syncing...' : 'New Quote'}
          </button>
          <button className="w-14 h-14 bg-white/5 rounded-2xl flex items-center justify-center text-[#A1A1AA]">
            <Share2 size={20} />
          </button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 gap-4">
        <div className="bg-[#1C1C1E] p-6 rounded-[2rem] flex flex-col justify-between border border-white/5 shadow-lg">
          <div className="flex justify-between items-start mb-4">
            <span className="text-[9px] text-[#A1A1AA] font-bold uppercase tracking-widest">Finished</span>
        ...
        <div className="bg-[#1C1C1E] p-6 rounded-[2rem] flex flex-col justify-between border border-white/5 shadow-lg">
          <div className="flex justify-between items-start mb-4">
            <span className="text-[9px] text-[#A1A1AA] font-bold uppercase tracking-widest">Score</span>
            <Target size={14} className="text-[#C5A059]" />
          </div>
          <div>
            <span className="text-3xl font-bold text-[#C5A059]">{efficiency}%</span>
            <p className="text-[9px] text-zinc-600 font-bold uppercase tracking-widest mt-4">Completion Rate</p>
          </div>
        </div>
      </div>

      {activeFocus && (
        <div className="bg-[#1C1C1E] border border-white/5 p-5 rounded-3xl shadow-lg flex items-center justify-between">
          <div className="min-w-0 pr-4">
            <div className="flex items-center space-x-2 mb-1">
              <Zap size={12} className="text-[#3B82F6]" />
              <span className="text-[9px] font-bold text-[#A1A1AA] uppercase tracking-widest">Next Up</span>
            </div>
            <h3 className="text-base font-bold text-white truncate">{activeFocus.title}</h3>
          </div>
      ...
      <div className="space-y-4 px-1 pb-12">
        <h2 className="text-[10px] font-bold text-[#A1A1AA] uppercase tracking-widest">Your Tasks</h2>
